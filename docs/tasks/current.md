...
- **目标**：让普通保存稳定区分磁盘内容变化与目标缺失，并在后端建立绑定当前文档 ID、可信描述信息和完整编辑快照的待解决冲突状态，为后续重新加载、强制覆盖和缺失处理提供不可由前端伪造的契约。
- **范围**：扩展 Rust 冲突分类、DocumentStore 冲突状态及稳定错误映射；首次冲突不得更新可信指纹或清除未保存状态；未知、过期、重复或已解决请求必须在文件 I/O 前拒绝；覆盖对应 Rust 自动化测试。
- **非范围**：不实现重新加载、强制覆盖、聚焦缺失检查或最终用户提示界面。
- **依赖**：已完成的安全保存核心、普通保存和首次保存/另存为切片；遵守 `resolve-save-conflict.md` 的可信状态与安全字段约束。
- **实施要点**：先定义内容变化与目标缺失的内部分类，再建立冲突状态的创建、查询、失效和清理规则；前端不得提交路径、指纹、冲突类型或冲突令牌。
- **完成标准**：Rust 自动化测试覆盖两类冲突、可信状态提交与清理、过期文档 ID、重复解决和会话变化；运行本任务适用的格式化、检查与测试并记录真实结果。
- **当前进度**：已完成规格确认和任务重新拆分，尚未修改实现代码或执行本功能验证。
...
- **状态**：待开始
- **Feature Spec**：`docs/features/resolve-save-conflict.md`
- **目标**：普通保存发现磁盘内容变化时展示冲突界面，使用户可以安全取消或从后端可信路径重新加载完整磁盘版本。
- **范围**：交付取消、重新加载、稳定错误提示、完整二进制内容提交，以及冲突期间编辑和文件操作互斥；成功后更新可信描述并清除未保存状态，失败或取消时保留编辑快照和磁盘文件。
- **非范围**：不提供强制覆盖；不处理文件缺失的保留或关闭流程。
- **依赖**：完成“建立保存冲突分类与后端可信状态”。
- **完成标准**：自动化测试覆盖取消、重新加载成功、读取期间再次变化、消失、超限、无效编码、权限和 I/O 失败，以及过期结果和重复操作；相关前后端检查通过并记录。

### 接入内容冲突的强制覆盖

- **状态**：待开始
- **Feature Spec**：`docs/features/resolve-save-conflict.md`
- **目标**：用户明确确认后，以冲突时保留的完整编辑快照覆盖确认后的最新磁盘基线，同时继续执行现有全部文件安全保护。
- **范围**：接入强制覆盖操作、确认后的重新观测、best-effort 再次竞争复核、成功后的可信状态更新，以及只读、权限、编码、换行、大小和 I/O 失败保护。
- **非范围**：不处理文件缺失时重建原路径，不提供合并、差异或备份能力。
- **依赖**：完成“接入内容冲突的取消与重新加载”，复用其冲突界面和操作互斥状态。
- **完成标准**：自动化测试覆盖成功覆盖、确认后再次变化或消失、只读、不可编码、编码歧义、Mixed、超限、符号链接和失败保护；相关前后端检查通过并记录。
