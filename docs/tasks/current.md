...
- **目标**：把 Save As/Untitled Save 的主对话框改为直接展示文件名与保存位置，编码与换行由应用主界面右下角的可选设置决定，并在不放宽文件安全边界的前提下完成一次可保存的最小闭环。
- **范围**：应用内目标面板默认展示文件名与保存位置；文件名可编辑；保存位置可通过受信任入口选择；编码与换行根据应用主界面右下角当前选择自动确定；确认后复用既有安全保存流程；取消保持当前会话不变。
- **非范围**：最近目录持久化、收藏目录、批量另存为、多标签、自动追加扩展名、完整集成验收与 README 收尾。
- **依赖**：「另存为目标路径与文件名选择」Feature 已完成；`docs/features/save-as-inline-target-panel.md` 已确认。
- **拆分检查**：本任务只交付一个主要用户行为：从当前另存为入口进入新的目标面板并完成单文档保存；完整回归、macOS 真实交互矩阵和文档收尾拆到后续集成验收任务。
- **实施要点**：先明确 Rust 侧可信目标草稿或目录选择契约，避免前端获得宽泛文件系统能力；保持二进制 IPC、编码限制、冲突检测和只读保护不变；Mixed 换行不得静默自动保存。
- **完成标准**：前端测试覆盖默认目标展示、文件名编辑、位置选择取消、主界面右下角格式设置、自动格式选择和取消恢复；Rust 测试覆盖目标契约与当前原路径路由；`npm run check`、`cargo test --manifest-path src-tauri/Cargo.toml`、`npm run build`、`git diff --check` 通过。
