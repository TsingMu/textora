...
- 2026-08-12 用户希望 Preview 模式下左右两侧同步滚动：当源码编辑区上下滑动时，预览区应尽量滚动到对应源码位置；当预览区上下滑动时，源码区也应同步到对应 Markdown 段落附近。
- 同步滚动应只作用于 Markdown Preview 左右分栏，不作用于 WYSIWYG、独立 Mermaid 预览、非 Markdown 标签或 Preview 关闭状态。
- 首版同步可采用“块级近似定位”：基于 Markdown 源码块、预览 DOM 块或滚动比例建立映射，目标是让标题、段落、列表、表格、代码块和 Mermaid block 附近对齐；不要求逐字符、逐像素精确。
- 需要避免双向滚动事件互相触发造成抖动或循环。用户正在主动滚动的一侧应短暂作为主控侧，另一侧跟随；程序触发的跟随滚动不得反过来立即抢占主控。
- 源码编辑、预览重新渲染、Mermaid 异步渲染、标签切换、窗口尺寸变化和大文档场景下，同步滚动不得阻塞编辑或保存；无法可靠映射时可安全退化为按滚动比例同步或暂不同步。

## 验收条件

- [x] Markdown 文件工具栏显示 `Preview` 开关；开启后进入左右分栏，左侧源码可正常编辑，右侧显示本地渲染结果。
- [x] 编辑标题、列表、任务列表、引用、代码块、表格和删除线等首版结构后，预览能跟随当前源码更新。
- [x] 保存后磁盘内容仍为 Markdown 源码，不包含预览产物或格式化副作用。
...
- [x] macOS release 应用真实验收覆盖打开 Markdown、编辑后预览更新、保存源码、多标签切换和关闭保护。
