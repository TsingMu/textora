...
实现、自动化验证与 macOS 交互验收已完成（2026-07-20）。根据 D-007，Windows 不再是产品运行或验收平台；Windows 来源文件兼容由确定性的编码与换行夹具验证。
...
- 本功能只要求 macOS 运行与原生文件对话框行为；Windows 构建、启动与原生对话框验证已由 D-007 取消，不再作为待验证项。
