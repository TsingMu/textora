...
- 2026-08-24 修复中文输入法预编辑文本被宽字素单元格折行：macOS WebKit 可能把原生 IME 组合文本临时放入光标相邻的 `.cm-wide-display-cluster` mark；原固定 `width: 2ch` 会把 `piliang` 等拼音按两字符宽度折成竖列。宽字素样式收进 CodeMirror 主题并改为 `min-width: 2ch`，使单个已提交宽字素仍至少占两列，而组合文本所在盒子可横向扩展；未在 composition 生命周期主动重建 DOM。定向测试通过；`npm run check`（**513 passed / 0 failed**）、`npm run build`、`./script/build_and_run.sh --verify` 与 `git diff --check` 通过。Tauri dev 应用已在真实 macOS WebKit 中成功启动并加载修复；Computer Use 无法切换全局输入源且过滤非 ASCII 注入，中文候选窗需用户手动复验。
- 2026-08-24 中文输入法修复 release 已完成部署：`npm run tauri -- build` 生成 `src-tauri/target/release/bundle/macos/Textora.app`；bundle 为 `com.tsingmu.textora`、可执行文件为 `textora`，经本地 ad-hoc 签名和严格校验后部署到 `/Applications/Textora.app`。安装版本再次通过严格签名校验，安装前后可执行文件 SHA-256 一致；`/usr/bin/open -n /Applications/Textora.app` 启动成功，进程名为 `textora`。旧安装版本保留在 `/private/tmp/Textora.app.codex-deploy-backup` 作为临时回滚副本。
