...
- 用户在单一空光标下，于有效 opening fence 行末按 Enter 时，若该 opening fence 尚无匹配 closing fence，则自动插入空内容行和匹配的 closing fence，并把光标放在内容行。
- 自动生成的 closing fence 保持 opening fence 的字符、长度和行首缩进。
- Markdown 源码单栏和 Preview 左侧源码编辑器使用同一行为；WYSIWYG 模式不执行自动闭合或 JSON 格式化。
...
1. 用户在 Markdown 源码中输入 opening fence，例如 <code>```json</code>。
2. 用户在该行末按 Enter。
...
- 自动闭合只接管单一空光标的 Enter。存在非空选区或多个选择时交给 CodeMirror 默认行为，避免改变列块编辑语义。
...
- [x] Markdown 源码中输入 opening fence 后按 Enter 会补齐 closing fence 并把光标放到内容行；已有 closing fence、非空选区、多选择和非 Markdown 文档不接管默认 Enter。
