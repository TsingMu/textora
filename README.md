...
项目已建立 Tauri 2、React、TypeScript、Vite、CodeMirror 6 与 Rust 可运行工程基线。当前可新建、打开和普通保存文档；首次保存与另存为使用应用内目标面板直接选择文件名和位置，编码与换行由主界面右下角设置决定。文件打开、普通保存、首次保存、另存为、保存冲突解决、未保存关闭保护（窗口关闭与应用退出）以及另存为内嵌目标面板均已完成自动化及 macOS 交互验收；多标签规格已确认，后端多文档可信状态、前端标签会话骨架以及打开/保存按活动标签绑定已完成。下一步协调窗口关闭与应用退出时逐一处理多个未保存标签。列块编辑和 Markdown 模式仍在 Backlog。
...
- `docs/features/multi-tab-session.md`：已确认的多标签会话规格
