...
项目已建立 Tauri 2、React、TypeScript、Vite、CodeMirror 6 与 Rust 可运行工程基线。当前可新建和打开文档，也已实现严格 UTF-8/CP936 识别、二进制 IPC、前端编辑会话与已打开文件的普通保存；文件打开和普通保存均已完成 macOS 验收。当前已确认下一切片“另存为与新建文档首次保存”的范围，尚未开始实现；Windows 验证仍待对应环境执行。
...
- `docs/features/save-as-and-first-save.md`：当前已承诺的另存为与新建文档首次保存规格
